...
const [loading,setloading]=useState(null)
  ...
  const categ = cat.map(bt => {
    return (
      ...
      <button onClick={ ()=>{getProdCat(bt)}} key={bt} className="bt-cat">{ bt}</button>
...
{categ.length>2&&  <button className="bt-cat"onClick={getProducts} key={Math.random()}>All</button>   }
            {products.length>2&&categ}
            </div>
        {loading&&<p className=" mt-5 spinner"></p>}
